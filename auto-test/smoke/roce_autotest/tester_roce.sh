...
#. ${TESTER_ROCE_TOP_DIR}/config/roce_test_config
#. ${TESTER_ROCE_TOP_DIR}/config/roce_test_lib
...
{
  list=""
#  file=`cat ${TESTER_ROCE_TOP_DIR}/data/roce_test_case.table`
  # | while read line
  while read line
  do
  ...
  done < ${TESTER_ROCE_TOP_DIR}/data/roce_test_case.table
  #echo $list
  ...
  sed -i 's/ /|/g'  table
  mv table ${PLINTH_BASE_WORKSPACE}/user/${T_TESTER}/roce/
...
{
cat <<EOF    
Usage: ./roce_autotest/tester_roce.sh [options]
Options:
...
echo -e "Tester: \033[34m hehui\033[0m \033[35m  zhangxiping\033[0m "
echo ">---------------------------------------------------------< "  
echo "  "
...
if [ -f ${PLINTH_BASE_WORKSPACE}/user/${T_TESTER}/roce/table ];then
    cp ${PLINTH_BASE_WORKSPACE}/user/${T_TESTER}/roce/table ${TESTER_ROCE_TOP_DIR}/data/roce_test_case.table
else
    echo ">--------------------------------------------------------------------------------<"
